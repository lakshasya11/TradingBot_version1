...
def test_complete_dynamic_trailing():
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        if active:
            ...
            if abs(actual_gap - expected_gap) < 0.01:
                print(f"  [PASS] PASS: Correct dynamic gap ({actual_gap:.1f})")
            else:
                print(f"  [FAIL] FAIL: Wrong gap (expected {expected_gap:.1f}, got {actual_gap:.1f})")
        else:
            print(f"  Status: {label}")
            if scenario['should_activate']:
                print(f"  [FAIL] FAIL: Should be active but isn't")
            else:
                print(f"  [PASS] PASS: Correctly not active")
    ...
    if trail_sl3 > trail_sl2 > trail_sl1:
        print(f"[PASS] RATCHETING WORKS: SL moved up progressively")
        print(f"   {trail_sl1:.5f} -> {trail_sl2:.5f} -> {trail_sl3:.5f}")
    else:
        print(f"[FAIL] RATCHETING FAILED: SL didn't move up properly")
    ...
    if gap3 < gap2 < gap1:
        print(f"[PASS] GAP TIGHTENING WORKS: {gap1:.1f} -> {gap2:.1f} -> {gap3:.1f}")
    else:
        print(f"[FAIL] GAP TIGHTENING FAILED: Gaps didn't tighten properly")
    
    print(f"\n[SUCCESS] DYNAMIC TRAILING STOP IS READY FOR LIVE TRADING!")
