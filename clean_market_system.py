...
def market_realistic_system():
    """Market-Realistic Trading System - 2-3 ticks per second"""
    ...
    print("=" * 60)
    print("Processing genuine new ticks only (2-3 per second)")
    ...
    try:
        while True:
            ...
            if tick and tick.time > last_tick_time:
                ...
                # Show tick info every 3rd tick
                if tick_count % 3 == 0:
                    print(f"\\n[TICK #{tick_count}] Rate: {tick_rate:.1f} ticks/sec")
                ...
                if rates is not None and len(rates) >= 2:
                    ...
                    market_positive = signal == "BUY"
                    market_negative = signal == "SELL"
                    ...
                    if analysis:
                        ...
                        if 'error' not in market_data:
                            ...
                            volume, volume_msg_order = entry_manager.order_logic.calculate_position_size(symbol, entry_price, stop_loss)
                ...
                # No new tick - realistic market timing (2-3 ticks/sec = 333-500ms)
                time.sleep(0.4)
            
    except KeyboardInterrupt:
        elapsed = time.time() - start_time
        actual_rate = tick_count / elapsed if elapsed > 0 else 0
        print(f"\\n\\nAnalysis stopped by user")
        print(f"PERFORMANCE SUMMARY:")
        print(f"  Runtime: {elapsed:.1f}s")
        print(f"  Genuine Ticks: {tick_count}")
        print(f"  Actual Rate: {actual_rate:.1f} ticks/sec")
        print(f"  Market Realistic: {'YES' if 2.0 <= actual_rate <= 4.0 else 'NO'}")
    finally:
        mt5.shutdown()

if __name__ == "__main__":
    market_realistic_system()